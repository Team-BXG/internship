...
const ApproveProblem = ({ selectedScope }) => {
  ...
  const fetchProblems = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || "http://localhost:8000")}/api/problems`);
  ...
  const handleStatusUpdate = async (problem, newStatus) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || "http://localhost:8000")}/api/problems/${problem.id}/status`, {
        method: 'PUT',
