...
def get_dashboard_data(db: Session, zone: str = None, woreda: str = None, gender: str = None, equipment_type: str = None, guarantee: str = None):
    # Base queries
    ...
    total_beneficiaries = b_query.filter(models.Beneficiary.status == "Approved").count()
    ...
    # Pending woreda approvals (beneficiaries awaiting first-level review)
    pending_approvals = b_query.filter(models.Beneficiary.status == "Pending").count()
    
    # Non-functional: woreda-approved problems not yet fixed
    non_functional_systems = p_query.filter(
    ...
    prev_pend = b_query.filter(models.Beneficiary.status == "Pending", models.Beneficiary.created_at >= sixty_days_ago, models.Beneficiary.created_at < thirty_days_ago).count()
    pending_trend = calc_trend(curr_pend, prev_pend)
    
    curr_prob = p_query.filter(models.Problem.status.in_(["Approved", "Seen"]), models.Problem.created_at >= thirty_days_ago).count()
    prev_prob = p_query.filter(models.Problem.status.in_(["Approved", "Seen"]), models.Problem.created_at >= sixty_days_ago, models.Problem.created_at < thirty_days_ago).count()
    ...
    if equipment_type:
        ben_trend_query = ben_trend_query.filter(models.Beneficiary.equipment_type == equipment_type)
    if guarantee:
        ben_trend_query = ben_trend_query.filter(models.Beneficiary.details_json.like(f'%{guarantee}%'))
    ...
    if equipment_type:
        eq_query = eq_query.filter(models.Beneficiary.equipment_type == equipment_type)
    if guarantee:
    ...
    if equipment_type:
        bz_query = bz_query.filter(models.Beneficiary.equipment_type == equipment_type)
    if guarantee:
        bz_query = bz_query.filter(models.Beneficiary.details_json.like(f'%{guarantee}%'))
    ...
    equipment_options = [
        row[0] for row in db.query(models.Beneficiary.equipment_type).distinct().all()
        if row[0]
    ]
