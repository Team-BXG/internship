...
export default function ZoneDashboard({ selectedZone }) {
  ...
  useEffect(() => {
    ...
    // Fetch Beneficiaries
    fetch((import.meta.env.VITE_API_URL || 'http://localhost:8000') + '/api/beneficiaries?approved_only=true')
      .then(res => res.json())
      .then(data => {
    ...
    // Fetch Problems
    fetch((import.meta.env.VITE_API_URL || 'http://localhost:8000') + '/api/problems?approved_only=true')
      .then(res => res.json())
      .then(data => {
